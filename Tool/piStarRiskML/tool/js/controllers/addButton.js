...
uiC.createAddButtons = function() {
	...
	new uiC.AddButtonView({
        model: new uiC.AddButtonModel({
            label: 'Event',
            action: ui.STATE_ADD_EVENT,
			name: 'Event',
            tooltip: 'Add Risk Event (Risk ML)',
            statusText: 'Click on a goal, quality, task or resource to add a Event'
        })
    }).render();
    new uiC.AddButtonView({
        model: new uiC.AddButtonModel({
    ...
    new uiC.AddButtonDropdownItemView({
	 ...
	 new uiC.AddButtonView({
        model: new uiC.AddButtonModel({
            ...
            statusText: 'Impact link: click first on the child, and then on the parent. It can only be applied to goal, quality, task or resource.'
        })
    }).render();
}