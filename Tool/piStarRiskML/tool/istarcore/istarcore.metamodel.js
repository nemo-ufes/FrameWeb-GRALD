...
var istarcoreMetamodel = {
    ...
    nodeLinks: [
        ...
        },
		{
            ...
            'isValid': function (source, target) {
                //RiksML Event 
                //event->goal,  event->quality, event->task, event->resource
                //The event relationship relates a event to its
                //subject: a task, goal, quality or resource.

                var result = false;
                result = source.isKindOfEvent() && target.isKindOfInnerElement();
                // result = result && (source.get('type') == target.get('type'));
                result = result && (source != target);
                return result;
            },
