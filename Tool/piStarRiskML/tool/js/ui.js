...
var ui = function() {
    ...
    return {
		...
		currentStateIsAddKindOfEvent: function () {
            return this.currentState === this.STATE_ADD_EVENT;
        },
...
ui.defineInteractions = function () {
    ...
    istar.paper.on('blank:pointerdown', function (evt, x, y) {
		...
		if (ui.currentStateIsAddKindOfEvent()) {
            ui.addEventOnPaper(x, y);
        }
    });
    ...
    istar.paper.on('cell:pointerup', function (cellView, evt, x, y) {
        ...
        if (ui.currentStateIsAddNode()) {
            ...
            if (cellView.model.isKindOfActor()) {
                ...
                if (ui.currentAddingElement.match(/AndRefinementLink|OrRefinementLink|NeededByLink|QualificationLink|ContributionLink|DependencyLink|make|help|hurt|break|ImpactLink/)) {
                    if (ui.isLinkSourceUndefined()) {
                        cellView.highlight({blur: 10, color: 'blue'});
                        ui.linkSource = cellView;
                    } else {
                        ui.linkTarget = cellView;
                        if (ui.currentAddingElement.match(/AndRefinementLink|OrRefinementLink|NeededByLink|QualificationLink|ContributionLink|make|help|hurt|break|ImpactLink/)) {
							...
							else if (ui.currentAddingElement === 'ImpactLink') 
							{								
								ui.addLinkImpactEvent(ui.currentAddingElement, cellView);					
							}
...
ui.addLinkImpactEvent = function (newLink, targetCellView) {
    try {
        ui.linkTarget = targetCellView;
        if (istar.types[newLink].isValid(ui.linkSource.model, ui.linkTarget.model)) {
            istar[istar.PREFIX_ADD + ui.currentAddingElement](ui.linkSource.model, ui.linkTarget.model);
...
function addElementInPlace(clickedNode, callback, x, y) {
    ...
    var node;
    if (clickedNode.isKindOfActor() || clickedNode.isKindOfEvent() ) {
        node = callback(x, y);
